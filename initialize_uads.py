...
cursor.execute('CREATE TABLE Instructor('
               ...
               '    Name VARCHAR(30),'
               '    Dept VARCHAR(30),'
               '    FOREIGN KEY(Dept) REFRENCES Department(Program)'
               ')')
connection.commit()
cursor.execute('CREATE TABLE Located('
               ...
               '    FOREIGN KEY(Program) REFRENCES Department(Program),'
               '    FOREIGN KEY(Address) REFRENCES Building(address)'
               ')')
connection.commit()
...
cursor.execute('CREATE TABLE Takes('
               ...
               '    FOREIGN KEY(Class_ID) REFRENCES Class(Class_ID),'
               '    FOREIGN KEY(Course_ID) REFRENCES Course(Course_ID)'
               ')')
connection.commit()               
cursor.execute('CREATE TABLE In('
               '    Course_ID VARCHAR(15),'
               '    Class_ID CHAR(5),'
               '    Room_Number INTEGER,'
               '    Address VARCHAR(100),'
               ...
               '    FOREIGN KEY(Course_ID) REFRENCES Course(Course_ID),'
               '    FOREIGN KEY(Class_ID) REFRENCES Class(Class_ID),'
               '    FOREIGN KEY(Room_Number) REFRENCES Room(Room_Number),'
               '    FOREIGN KEY(Address) REFRENCES Building(address)'
               ')')
connection.commit()               
cursor.execute('CREATE TABLE TaughtBy('
               ...
               '    Course_ID VARCHAR(15),'
               '    PRIMARY KEY(ID, Class_ID, Course_ID),'
               '    FOREIGN KEY(ID) REFRENCES Instructor(ID),'
               '    FOREIGN KEY(Class_ID) REFRENCES Class(Class_ID),'
               '    FOREIGN KEY(Course_ID) REFRENCES Course(Course_ID)'
               ')')
connection.commit()               
